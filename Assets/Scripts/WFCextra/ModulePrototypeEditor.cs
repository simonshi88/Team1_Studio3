	...
	public override void OnInspectorGUI() {
		...
		if (GUILayout.Button("Distribute")) {
			...
			foreach (Transform transform in modulePrototype.transform.parent) {
				transform.localPosition = Vector3.forward * i * InfiniteMap.BLOCK_SIZE * 3f;
				i++;
			}
		}
		...
		if (GUILayout.Button("Distribute (Overview)")) {
			...
			int i = 0;
			foreach (Transform transform in modulePrototype.transform.parent) {
				transform.localPosition = Vector3.forward * (i / w) * AbstractMap.BLOCK_SIZE * 2f + Vector3.right * (i % w) * AbstractMap.BLOCK_SIZE * 2f;
				i++;
			}
		}
